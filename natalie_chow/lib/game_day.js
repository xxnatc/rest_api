...
module.exports = exports = (values, res) => {
  ...
  console.log(index, towns.length, mafias.length);
  if (index < towns.length) {
    // a town will to be lynched
    ...
    // a mafia will be lynched
    index -= towns.length;
    Mafia.remove({ _id: mafias[index]._id }, (err) => {
      if (err) return dbErrorHandler(err, res);
      res.status(200).json({ msg: `Town of Salem voted for ${mafias[index].name} to be lynched. ${mafias[index].age}-year-old ${mafias[index].name} was a ${mafias[index].rank} in the mafias.`});
    });
  }
};
